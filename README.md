...
| Facts in database | 15,112 |
| Years with ratios, per company | 3-8 (median 5.5); calendar span 2017-2025 |
| Ratios computed | 12 distinct ratios: 681 values, 303 blank (an input is missing or the ratio is not meaningful for that company - stored blank, never as zero) |
| Trading comps | 11/16 companies |
| 3-statement model | 9/16 companies |
| DCF valuation | 9/16 companies |
| Market risk | 11/16 companies |
| Credit profile | 11/16 companies |
| Ratio forecasts | 9/16 companies |
| Forecast backtest | 9/16 companies |
| Forensics flags | 69 across 13 companies (27 high / 13 medium / 29 low severity) |
| Candidate universe | 156 distinct entities over 4 snapshots (latest 2026-09-17); 0 linked to a loaded company |
| Tests | 318 (284 run in CI without a database, 34 need a live database and skip there) |
<!-- FACTS:END -->
...
- **Amplifon and Shell** have no 3-statement model or DCF: their filings report costs "by nature" with no COGS / gross-profit split, so no number is fabricated for them (see [Known limitations](#known-limitations)).
