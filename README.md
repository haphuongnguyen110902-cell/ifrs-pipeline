...
**Live demo:** [ifrs-pipeline dashboard](https://haphuongnguyen110902-cell-ifrs-pipeline-webappapp-iwnn9s.streamlit.app/) — filter by country/sector, browse ratios, earnings-quality flags, trading comps, a linked 3-statement model, and a DCF valuation for the European listed companies loaded (the current count is in the status table below).
...
- **Tests itself** — a pytest regression suite (size in the status table below) covering every bug found and fixed during development, run on every push via CI
...
| Ratios computed | 13 distinct ratios: 1,027 values, 360 blank (an input is missing or the ratio is not meaningful for that company - stored blank, never as zero) |
| Trading comps | 15/16 companies |
| 3-statement model | 15/16 companies |
| DCF valuation | 14/16 companies |
...
| Tests | 864 (825 run in CI without a database, 39 need a live database and skip there) |
<!-- FACTS:END -->
...
How to read the coverage rows - fewer than every company is deliberate or pending, not hidden:

- **Amplifon, Shell and Heineken** have no 3-statement model or DCF: their filings report costs "by nature" with no COGS / gross-profit split, so no number is fabricated for them (see [Known limitations](#known-limitations)).
- **Adyen** (a payment processor) has no trading comps or credit profile, and **ASM International** none either: Adyen's "net cash" is merchants' money, and ASM has no debt line stored yet, so its net debt is left blank rather than assumed.
- **Six companies show `n/a*` instead of an EBITDA multiple** (L'Oréal, Schneider, Recordati, EssilorLuxottica, LVMH, Kering): their statements do not print depreciation and amortisation separately, so EBITDA cannot be derived and a multiple built on EBIT would overstate leverage.
- **Net debt includes IFRS 16 lease liabilities.** Many companies' own "net financial debt" excludes them (LVMH 2024: about EUR 9.2bn as reported, about EUR 31bn here, of which EUR 17.8bn is leases).
- **Companies loaded from the universe after the first eleven** (ASM International, Adyen, Heineken, Recordati, Schneider Electric) have ratios and forensics but may not yet have every analysis stage - the coverage rows show how far each stage has got.
- **Financial companies** (Adyen): ratios that are meaningless for a lender, insurer or payment processor (gross margin, working capital, ROIC, EBITDA leverage) are stored blank with the reason recorded, not shown as a misleading number.
- **Credit profile** is the Net Debt / true-EBITDA trajectory; years with no D&A tag matched are flagged, not hidden.
- **Data checks** (`08_validate.py`, last run 2026-09-19): 5/5 known-good values match L'Oréal's published 2024 annual report - the only company with hand-verified known values so far; accounting identities 140/142, the 2 failures being Adyen's `Gross Profit = Revenue - Cost of sales` (a payment processor's cost lines do not reconcile to that identity; tracked in `PLAN.md`).
- **Dashboard:** live and public (link above), 9 tabs per company, analysis layer refreshed weekly by CI.

See [`PLAN.md`](PLAN.md) for the current work-package status and the audit findings, and [`ROADMAP.md`](ROADMAP.md) for the phase-by-phase engineering log - every real bug found along the way and how it was fixed.
...
python scripts/12_apply_review.py
python scripts/09_batch_load.py            # loads every zip in data/raw; re-loading is idempotent, so this is safe to repeat
```

Do **not** add `--reset-facts` to that: it deletes existing facts before reloading and is only for a mapping change that must *remove* facts. It is not needed to avoid duplicates (the loader upserts on `filing, period, concept`).
...
| `05_load_data.py` | First-generation loader: parsed-facts CSV + concept mapping into the database (`09_batch_load.py` is the one to use now) |
| `07_generate_statements.py` | Render a company's income statement / balance sheet / cash flow from the stored facts |
...
| `download_historical.py` | Download historical ESEF filings from filings.xbrl.org (one per fiscal year) for `load_historical.py` |
| `load_historical.py` | Load historical multi-year filings (V2), separate filing row per company/year |
| `10_auto_classify.py` | Classify a filing's extension tags using the IFRS presentation linkbase |
...
| `14_scan_universe.py` | Scan filings.xbrl.org for a country's ESEF filers and report which are new or have a newer filing than the database |
...
| `33_load_note_facts.py` | Loads figures a company prints in its notes but does not tag (D&A of LVMH, L'Oréal, Essity) from a reviewed specification; every figure is read from the report by code and refused unless its checks pass |
...
| `tests/` | pytest regression suite (size in the status table) — see each file's docstring for the real bug it locks in |
