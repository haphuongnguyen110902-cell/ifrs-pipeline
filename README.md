...
- **Serves** a public Streamlit dashboard (`webapp/app.py`) with 8 tabs per company — ratios, forensics flags, trading comps, the 3-statement model, DCF valuation, market risk, forecast backtest results, and precedent transactions
- **Refreshes automatically** every Monday via GitHub Actions (`--mode analyze`, no local files needed — see [Automation](#automation))
- **Tests itself** — 101 pytest regression tests covering every bug found and fixed during development, run on every push via CI
...
| Regression tests (code) | 101 pytest tests, run on every push via GitHub Actions CI |
...
| Dashboard | Live and public (see link above), 8 tabs per company, refreshed weekly by CI |
...
Statements   Ratios   Validation  Forensics   Forecast    Backtest
 (Excel)  (DB+Excel)   (5/5)     (DB+Excel)  (DB+Excel)  (DB+Excel)
        ...
        ┌───────────┼──────────────────┬──────────────────┬──────────────┐
        ↓           ↓                  ↓                  ↓              ↓
  Trading comps  3-statement    DCF valuation      Precedent      Market risk
  (19_valuation)  model (21_*)      (22_dcf)      transactions   (23_market_risk,
                                                       (20_*)      price history
                                                                   only - no `ratio`
                                                                   table dependency)
        └───────────┴──────────────────┴──────────────────┴──────────────┘
                    ...
                    (8 tabs per company - ratios, forensics, comps,
                     3-statement, DCF, market risk, backtest, precedents)
```
...
The valuation/market-risk layer (comps/3-statement/DCF/precedents/market
risk) is currently run by hand, not yet part of either pipeline mode -
...
- The comps/3-statement/DCF/precedents/market-risk layer (`19`/`20`/`21`/`22`/`23_*.py`) is run by hand, not yet wired into `run_pipeline.py`'s `--mode analyze` or the weekly cron — see the note under [Architecture](#architecture)
